...
app.get('/table', function (_req, res, _next) {
    pool.connect(function(err,client,done) {
       if(err){
           console.log("not able to get connection "+ err);
           res.status(400).send(err);
       } 
       client.query('SELECT * from account_details' ,function(req,result) {
    ...
    // for(int i=1;i<len;i++){
    //     console.log(stringfy.rows[i].ad_ch_acct_id);
    //     }
...
var table_search = require("./routes/table_search");
var table_add = require("./routes/table_add.js");
...
app.use('table_add',table_add)
app.get('/',function(_req,res){
    res.render('login');
    
});
app.get('/table/table_search', function(req,res){
    res.render('table_search');
});
app.get('/table/table_add', function(req,res){
    res.render('table_add');
});
...
app.get('/table', function(_req,res){
    pool.connect(function(err,client,done) {
        if(err){
            console.log("not able to get connection "+ err);
            res.status(400).send(err);
        } 
        client.query('SELECT * from account_details' ,function(req,res) {
            done(); // closing the connection;
            if(err){
                console.log("err");
                res.status(400).send();
            ...
            res.render('table', { name: account_name, date: account_number, amt: acc_status, install: acc_curr_balance });


          });
          res.render('table', { fj:result
